...
def get_user_input():
    ...
    query = input("Enter the term you would like to search on (leave blank for all) : ")
...
def main(city, state, query):
    ...
    for offset in offsets:
        raw_data = yelp_api.search_query(term=query, location=city_state, offset=offset, limit=50)
    ...
    df.to_csv("../data/" + city + "_"+query+"_"+current_date+".csv")

    # data cleaning and transformation
    ######################################################
    #Fix transactions
    df = pd.read_csv("../data/" + city + "_"+query+"_"+current_date+".csv")
    ...
    for category in category_df['category']:
        try:
            # Check if the category is a valid JSON string
            cat = json.loads(category)
            category_dict.append(cat)
        except (json.JSONDecodeError, TypeError):
            # Handle the case where the category is not a valid JSON string
            print(f"Skipping invalid JSON: {category}")
    ...
    print(query+" Data successfully pulled for: "+city+", "+state)
